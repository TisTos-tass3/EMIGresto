...
def test_ticket_crud(auth_client, create_user):
    ...
    # Create petit-déj
    resp = auth_client.post("/api/tickets/", {"typeTicket": "PETIT"})
    assert resp.status_code == 201
    tid = resp.data["idTicket"]
    assert resp.data["prixTicket"] == 80

    # Create dej/dîner
    resp2 = auth_client.post("/api/tickets/", {"typeTicket": "DEJ"})
    assert resp2.status_code == 201
    assert resp2.data["prixTicket"] == 125
    ...
    assert resp3.data["typeTicket"] == "PETIT"
