...
const r = {
    ...
    jourBoxPaddingH: 8 * widthRatio, // Padding inside each day box
    jourBoxPaddingV: 8 * heightRatio, // Padding inside each day box
    jourBoxMarginH: 4 * widthRatio, // Margin between day boxes
    jourFontSize: 14 * widthRatio,

    // Calculate dayBoxSize dynamically to fit 7 boxes with margins
    // The `View` wrapping the day boxes has `paddingHorizontal: r.jourBoxPaddingH / 2`.
    // So, total left/right padding for the row is (r.jourBoxPaddingH / 2) * 2 = r.jourBoxPaddingH = 8 * widthRatio
    // There are 6 margins between 7 boxes: 6 * r.jourBoxMarginH = 6 * 4 * widthRatio = 24 * widthRatio
    // Total horizontal space consumed by margins and row padding: 8 * widthRatio + 24 * widthRatio = 32 * widthRatio
    dayBoxSize: (screenWidth - (8 * widthRatio + 24 * widthRatio)) / 7,
